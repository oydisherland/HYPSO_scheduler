...
for target in targets:
    ...
    timestamps, types = skf_h1.find_events(target_location, t0, t0 + timewindow_days, altitude_degrees=float(elevation))
    ...
    for i in range(len(timestamps)):
        ...
        if types[i] == 2:
            utc_endtime = timestamps[i].utc_datetime().replace(microsecond=0) - timedelta(seconds=capture_time)
...
updated_targets_df.to_csv('updated_targets.csv', index=False)
